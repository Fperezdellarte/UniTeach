...
const ClassScheduleCalendar = ({ mentorId, searchTerm }) => {
    ...
    const fetchSchedule = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/classes/mentorclass/${mentorId}`,
          {
            params: {
  ...
  const getTileClassName = ({ date }) => {
    const classDates = schedule.map((item) =>
  ...
  return (
    <div className="calendar-container">
      ...
      {showSchedule && scheduleDetails.length > 0 && (
        ...
        >
          <button onClick={handleCloseSchedule} className="close-btn">
            ✕
          </button>
          <ul>
            {scheduleDetails.map((item) => (
              <li
                ...
                className={`schedule-item ${
                  selectedClassId === item.idClasses ? "selected" : ""
                }`}
      ...
      <Snackbar open={success} autoHideDuration={3000}>
        <Alert severity="success">Inscripción realizada con éxito.</Alert>
      </Snackbar>
    </div>
  );
};
